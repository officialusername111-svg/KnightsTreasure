...
// Bottom-nav sections. icon = base name in assets/images/ui (falls back to label-only
// until the art is generated).
const SECTIONS = [
...
// How coins are earned (GDD economy) — shown when the player taps the coins pill.
const COIN_SOURCES = [
...
export function createHomeScene({ gameState, onPlay, onSection }) {
  ...
  scene.id = 'kt-home';
  scene.style.setProperty('--bg-stage', `url("${STAGE_BG[stage] || ASSETS.bgForest}")`);
  ...
  scene.innerHTML =
    ...
    `<div class="kt-topbar">` +
      ...
      `<button type="button" class="kt-res" id="kt-res-coins"><img src="${ASSETS.ui}ui_coin.png" alt=""><span>${save.coins || 0}</span></button>` +
      `<button type="button" class="kt-icon-btn" id="kt-settings" aria-label="Menu"><svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="${MENU_ICON}"/></svg></button>` +
    `</div>` +
...
// Dismissible parchment info panel appended over the home scene.
